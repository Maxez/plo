...
static inline unsigned char console_read(void *base, unsigned int reg)
{
...
static inline void console_write(void *base, unsigned int reg, unsigned char val)
{
	unsigned int addr = (unsigned int)base;
...
void hal_consoleInit(void)
{
	...
	console_write(halconsole_common.base, DLL, UART_DIVISOR);
	console_write(halconsole_common.base, DLM, UART_DIVISOR >> 8);
